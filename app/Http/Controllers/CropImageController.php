...
class CropImageController extends Controller
{
    ...
    public function uploadCropImageDocument(Request $request)
    {
        ...
        if (!file_exists('storage/images/document/users/'.$participant->user_id.'/')) {
            mkdir('storage/images/document/users/'.$participant->user_id.'/', 0777, true);
        }
        $folderPath = public_path('storage/images/document/users/'.$participant->user_id.'/');
        $image_parts = explode(";base64,", $request->image);
        $image_type_aux = explode("image/", $image_parts[0]);
        $image_type = $image_type_aux[1];
        $image_base64 = base64_decode($image_parts[1]);
        if ($image_type == 'png'|| $image_type == 'jpg' || $image_type == 'jpeg')
        {
            ...
            $imageFullPath = $folderPath.$imageName;
            file_put_contents($imageFullPath, $image_base64);
            $participant->document = 'images/document/users/'.$participant->user_id.'/'.$imageName;
            if($participant->save()){
                return response()->json(['success'=> true,'message' => 'Документ успешно приложен'], 200);
            }
        } else {
            return response()->json(['success'=> false, 'message' => 'Неверный формат файла'],422);
        }
