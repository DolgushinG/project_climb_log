...
//                    $participant = ResultQualificationClassic::where('event_id','=',$event->id)->where('user_id','=',$user_id)->first();
            ...
            $participant_helper_amount = $participant->helper_amount ?? null;
            $current_amount_start_price = OwnerPaymentOperations::current_amount_start_price_before_date($event);
            return view('welcome', compact(['participant_helper_amount','current_amount_start_price','participant_products_and_discounts','message_for_participants','event','google_iframe','count_participants','is_show_button_list_pending','list_pending','is_add_to_list_pending', 'sport_categories', 'sets', 'is_show_button_final',  'is_show_button_semifinal']));
        } else {
            return view('404');
        }
    }
