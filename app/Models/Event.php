...
class Event extends Model
{
    ...
    public static function refresh_qualification_counting_like_final($event)
    {
        ...
        if ($event->is_sort_group_final) {
            ...
            foreach ($all_group_participants as $group_participants) {
                foreach ($group_participants as $participants) {
                    ResultRouteSemiFinalStageController::getUsersSorted($participants, $fields, $event, 'france_system_qualification', Admin::user()->id);
                }
            }
        } else {
            ...
            ResultRouteSemiFinalStageController::getUsersSorted($participants_female, $fields, $event, 'france_system_qualification', Admin::user()->id);
            ResultRouteSemiFinalStageController::getUsersSorted($participants_male, $fields, $event, 'france_system_qualification', Admin::user()->id);
        }
    }
